#!/usr/bin/env python3
"""
SAM-HQ Video Auto Segmentation Test
동영상에서 300프레임마다 SAM-HQ 세그멘테이션을 수행하고 결과를 시각화
"""
...
def process_video(video_path, output_dir, frame_interval=300):
    """
    동영상을 읽어서 지정된 간격마다 SAM-HQ 세그멘테이션 수행
    ...
    """
    print("=" * 80)
    print("SAM-HQ Video Auto Segmentation Test")
    print("=" * 80)
    ...
    # SAM-HQ 모델 로드
    print("\n[INFO] Loading SAM-HQ model...")
    checkpoint_path = "/workspace/models/sam_hq_vit_l.pth"
    model_type = "vit_l"
    ...
    try:
        ...
        print("[INFO] ✓ SAM-HQ model loaded successfully")
        ...
        print(f"[ERROR] Failed to load SAM-HQ model: {e}")
        import traceback
        traceback.print_exc()
        return
    ...
    # 프레임 처리
    print("\n[INFO] Processing frames with SAM-HQ...")
    ...
    for frame_idx in tqdm(frames_to_process, desc="Processing"):
        ...
        try:
            ...
            axes[1].set_title(f'SAM-HQ Segmentation ({len(masks)} masks)', fontsize=16)
            axes[1].axis('off')
            
            # 저장
            output_path = os.path.join(output_dir, f'frame_{frame_idx:06d}_samhq_seg.png')
            plt.tight_layout()
            ...
            if "out of memory" in str(e).lower():
                print(f"[ERROR] OOM Error at frame {frame_idx}!")
                print(f"[ERROR] {e}")
                ...
                if torch.cuda.is_available():
                    allocated = torch.cuda.memory_allocated(0) / 1024**3
                    reserved = torch.cuda.memory_reserved(0) / 1024**3
                    print(f"[INFO] VRAM at failure:")
                    print(f"       - Allocated: {allocated:.2f} GB")
                    print(f"       - Reserved: {reserved:.2f} GB")
                
                print("[INFO] Stopping test due to OOM error")
                break
            else:
                raise
...
def main():
    ...
    result_dir = "/workspace/test_codes/result_samhq"
