#!/usr/bin/env python3
"""
SAM2 Large Model - Video Auto Segmentation Test
...
def process_video(video_path, output_dir, frame_interval=300):
    ...
    """
    print("=" * 80)
    print("SAM2 LARGE Model - Video Auto Segmentation Test")
    print("=" * 80)
    ...
    try:
        ...
        print("[INFO] ✓ SAM2 LARGE model loaded successfully")
        
        if torch.cuda.is_available():
            allocated = torch.cuda.memory_allocated(0) / 1024**3
            reserved = torch.cuda.memory_reserved(0) / 1024**3
            print(f"[INFO] VRAM after load:")
            print(f"       - Allocated: {allocated:.2f} GB")
            print(f"       - Reserved: {reserved:.2f} GB")
        
    except Exception as e:
        print(f"[ERROR] Failed to load SAM2 LARGE model: {e}")
        import traceback
        traceback.print_exc()
        return
    ...
    # 프레임 처리
    print("\n[INFO] Processing frames with LARGE model...")
    ...
    for frame_idx in tqdm(frames_to_process, desc="Processing"):
        ...
        try:
            ...
            if torch.cuda.is_available() and frame_idx == frames_to_process[0]:
                ...
                if remaining < 0:
                    print(f"       ⚠️  WARNING: VRAM overcommitted by {abs(remaining):.2f} GB!")
            ...
            axes[1].set_title(f'SAM2 LARGE Segmentation ({len(masks)} masks)', fontsize=16)
            axes[1].axis('off')
            
            # 저장
            output_path = os.path.join(output_dir, f'frame_{frame_idx:06d}_large_seg.png')
            plt.tight_layout()
...
def main():
    ...
    result_dir = "/workspace/test_codes/result_large"
